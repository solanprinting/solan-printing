...
(function (root, factory) {
  ...
  /* ── אבחון-כשל לקישור הקצר ─────────────────────────────────────────
     ⚠️ **התקלה שדווחה (10/08/2026): לקוח לא הצליח לפתוח את הפורטל
     וקיבל "הקישור לא נמצא · אין חיבור כרגע".** נמדד מול הייצור: הקריאה
     עם אסימון אנונימי מחזירה 200, כלומר הקישור והשרת תקינים לגמרי.
     מה שקרה הוא ש-‎fetch‎ **נזרק** אצל הלקוח — כלומר הדומיין לא נגיש
     ממכשירו (סינון-רשת, אינטרנט מסונן, נתק).

     ⚠️ והכותרת שיקרה. "הקישור לא נמצא" שולחת את הלקוח לבקש קישור חדש
     מבית-הדפוס — קישור שגם הוא ייכשל, כי הבעיה אינה בקישור. שלוש
     סיבות שונות לגמרי התמזגו לכותרת אחת שגויה.

     מחזיר { title, msg, blocked } לפי מה שבאמת קרה. */
  function failInfo(o){
    var s = o || {};
    if (s.badCode) {
      return { title: 'הקישור אינו תקין', blocked: false,
               msg: 'נראה שהכתובת הועתקה חלקית. בקשו קישור מעודכן מבית הדפוס.' };
    }
    /* ⚠️ זריקה = לא הצלחנו **להגיע** לשרת. זה אינו "לא נמצא". */
    if (s.threw) {
      return { title: 'אין גישה לשרת', blocked: true,
               msg: 'החיבור לשרת נחסם או שאין רשת. אם אתם על אינטרנט מסונן — '
                  + 'נסו מרשת אחרת (לדוגמה נתוני-סלולר), או פנו לבית הדפוס.' };
    }
    if (s.status === 401 || s.status === 403) {
      return { title: 'אין גישה לקישור', blocked: true,
               msg: 'ההרשאה לקישור הזה נדחתה. פנו לבית הדפוס לקישור מעודכן.' };
    }
    if (s.status && s.status >= 500) {
      return { title: 'השרת אינו זמין', blocked: true,
               msg: 'תקלה זמנית בשרת. נסו שוב בעוד דקה.' };
    }
    /* רק כאן זה באמת "לא נמצא": הגענו לשרת, והוא ענה שאין רשומה. */
    return { title: 'הקישור לא נמצא', blocked: false,
             msg: 'הקישור פג או שאינו קיים. בקשו קישור מעודכן מבית הדפוס.' };
  }
  ...
  return { ALPHABET: ALPHABET, CODE_LEN: CODE_LEN, NODE: NODE,
           ...
           safeTarget: safeTarget, pretty: pretty, failInfo: failInfo };
});
