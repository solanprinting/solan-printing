...
(function (root, factory) {
  ...
  /* ⚠️ **דיווח-בעלים חוזר 07/09/2026** ("עוד פעם הגיליון הקודם מופיע בתאב
     ליד הגיליון החדש"): גיליון 279 של השבועון-אלעד נשאר בלשוניות שבועיים,
     ליד 281. בייצור מצאתי למה — ‎closedAt=24/08‎ ו-‎printApprovedAt‎ **ריק**:
     הוא נסגר להעלאה ומעולם לא נלחץ עליו "אושר להדפסה", ולכן הכלל הישן
     ("הודפס או הושלם") פסח עליו.

     זו הייתה **סתירה בתוך המודול עצמו**: ‎isActive‎ כבר סופר גיליון סגור
     כלא-פעיל, ו-‎openId‎ מדלג עליו כשהוא בוחר איזו לשונית לפתוח — אבל
     ‎splitPast‎ המשיך להציג אותו כלשונית נוכחית. שתי הגדרות ל"פתוח".

     ⚠️ הסגירה לבדה אינה "נגמר": סוגרים להעלאה בבוקר ומדפיסים אחר-הצהריים,
     וגיליון שנעלם מהמסך באמצע-העבודה גרוע מגיליון-עודף. לכן גיליון סגור
     שאין עליו אישור-הדפסה עובר לתיקייה רק אחרי יממה — אז זו כבר היסטוריה.
     גיליון שאושר-להדפסה או הושלם עובר מיד, כמו קודם.

     ‎now‎ מוזרק כדי שהחלון ייבדק, ולא ייבדק "בערך". */
  var PAST_GRACE_MS = 24 * 60 * 60 * 1000;
  function splitPast(list, now) {
    ...
    rows.forEach(function (p, i) {
      var done = printApproved(p) || num(p.completedAt) > 0;
      var closedLongAgo = num(p.closedAt) > 0 && (t - num(p.closedAt)) >= PAST_GRACE_MS;
      if (i > 0 && (done || closedLongAgo)) past.push(p); else current.push(p);
    });
  ...
  /* ⚠️ **הפריסה כאן היא קונטרס תפור בלבד** — ריצה עוטפת ריצה, ולכן כל
     ריצה נושאת שני מקטעים (‏1-16 + 81-96) פרט לפנימית, שהיא רציפה.
     ⚠️ **חוברת-הדבקה עובדת אחרת לגמרי**: שם הריצות בסדר רץ (1-32 · 33-64 ·
     65-96) והחישוב כאן ייתן מספרים **שגויים**. הבעלים אישר במפורש
     (24/08/2026) שהדבקה נדירה אצלנו ו**אינה נכנסת למערכת כרגע** — נרשם
     כאן כדי שמי שיוסיף אותה בעתיד יידע שזו נקודת-הפיצול, ולא יגלה זאת
     מקובץ שיצא לדפוס עם מספור של השיטה השנייה. [[solan-glued-binding-future]] */
